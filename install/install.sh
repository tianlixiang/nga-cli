#!/bin/sh
# NGA CLI — macOS / Linux Installer / Updater
# Usage:   curl -fsSL https://ngacli.com/install.sh | sh
# License: AGPL-3.0-or-later (https://github.com/edison7009/Coffee-CLI/blob/main/LICENSE)

set -e

# Resolve version and binary via ngacli.com (CF-hosted, China-accessible).
...
# networks.
VERSION_BASE="https://ngacli.com/version.json"
DOWNLOAD_BASE="https://ngacli.com/download"
# Direct GitHub Releases fallback used when ngacli.com is unreachable
...
echo ""
echo "  ${CYAN}NGA CLI Installer${RESET}"
echo "  ${GRAY}────────────────────${RESET}"
...
if [ "$OS" = "Darwin" ]; then
  ...
  case "$ARCH" in
    ...
    *)
      ...
      echo "  ${YELLOW}NGA CLI ships arm64 and Intel x64 macOS builds.${RESET}"
  ...
  case "$ARCH" in
    ...
    *)
      ...
      echo "  ${YELLOW}NGA CLI currently ships amd64 and arm64 Linux builds only.${RESET}"
      echo "  ${YELLOW}Open an issue: https://github.com/edison7009/Coffee-CLI/issues${RESET}"
      exit 1
      ;;
  esac
...
# ngacli.com unreachable (CF Worker 502 / network) → fall back to
...
# for the later download step (so we don't hit ngacli.com a second
# time and fail again).
if [ -z "$LATEST_VER" ] || [ "$LATEST_VER" = "$VERSION_JSON" ]; then
  echo "  ${GRAY}Trying GitHub directly...${RESET}"
  GH_JSON=$(curl -fsSL -H "User-Agent: NGACLI-Install" "$GITHUB_API" 2>/dev/null || true)
  if [ -n "$GH_JSON" ]; then
...
if [ -z "$LATEST_VER" ] || [ "$LATEST_VER" = "$VERSION_JSON" ]; then
  echo ""
  echo "  ${YELLOW}A new version of NGA CLI was just released.${RESET}"
...
if [ "$OS" = "Darwin" ]; then
  ...
  INSTALLED_VER=""
  APP_PATH="/Applications/NGA CLI.app"
  if [ -d "$APP_PATH" ]; then
  ...
  if [ -n "$INSTALLED_VER" ]; then
    ...
    if [ "$INSTALLED_VER" = "$LATEST_VER" ]; then
      echo ""
      echo "  ${GREEN}NGA CLI is already up to date (v$INSTALLED_VER).${RESET}"
      echo ""
      exit 0
    fi
  ...
  TMP="/tmp/nga-cli-v${LATEST_VER}.dmg"
  ...
  # when the volume name has spaces (e.g. "NGA CLI 1.6.4"), since $NF
  ...
  xattr -dr com.apple.quarantine "/Applications/NGA CLI.app" 2>/dev/null || true

  echo ""
  echo "  ${GREEN}Done! NGA CLI v$LATEST_VER installed.${RESET}"
...
# ── Linux ──────────────────────────────────────────────────────────────────────
  ...
  INSTALLED_VER=""
  if command -v dpkg > /dev/null 2>&1; then
    INSTALLED_VER=$(dpkg -s nga-cli 2>/dev/null | grep '^Version:' | sed 's/Version: //' || true)
  fi
  if [ -z "$INSTALLED_VER" ] && command -v rpm > /dev/null 2>&1; then
    INSTALLED_VER=$(rpm -q --queryformat '%{VERSION}' nga-cli 2>/dev/null || true)
  fi
  ...
  if command -v rpm > /dev/null 2>&1; then
    TMP="/tmp/nga-cli-v${LATEST_VER}.rpm"
    rm -f "$TMP"
  ...
  # failed partway.
  TMP="/tmp/nga-cli-v${LATEST_VER}.AppImage"
  rm -f "$TMP"
  ...
  echo ""
  echo "  ${GREEN}Done! NGA CLI v$LATEST_VER installed to $DEST${RESET}"
