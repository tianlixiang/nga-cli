# NGA CLI - Windows Installer / Updater
# Usage:   irm https://ngacli.com/install.ps1 | iex
# License: AGPL-3.0-or-later (https://github.com/edison7009/Coffee-CLI/blob/main/LICENSE)

$ErrorActionPreference = "Stop"

Write-Host ""
Write-Host "  NGA CLI Installer" -ForegroundColor Cyan
...
# Try ngacli.com first (CF-cached, China-accessible). If the Worker
...
if (-not $latestVer) {
    try {
        $release = Invoke-RestMethod "https://api.github.com/repos/edison7009/Coffee-CLI/releases/latest" `
            -Headers @{ "User-Agent" = "NGACLI-Install" } -TimeoutSec 15
...
foreach ($path in $regPaths) {
    $entry = Get-ItemProperty $path -ErrorAction SilentlyContinue |
             Where-Object { $_.DisplayName -like "NGA CLI*" } |
...
if (-not $latestVer) {
    Write-Host ""
    Write-Host "  A new version of NGA CLI was just released." -ForegroundColor Yellow
...
if ($installedVer) {
    ...
    if ($installedVer -eq $latestVer) {
        Write-Host ""
        Write-Host "  NGA CLI is already up to date (v$installedVer)." -ForegroundColor Green
        Write-Host ""
        exit 0
    }
...
$url = if ($fallbackUrl) { $fallbackUrl } else { "https://ngacli.com/download/windows" }
$out = "$env:TEMP\nga-cli-setup.exe"
...
Write-Host ""
Write-Host "  Done! NGA CLI v$latestVer installed." -ForegroundColor Green
